...
class XfunReTrainer(FunsdTrainer):
    ...
    def prediction_loop(
        ...
        gt_relations = []    # gt_relation中的实体，实体识别模型可能预测不出来。因此根据关系获取预测的实体时，可能为空
        for b in range(len(re_labels)):
            rel_sent = []
            # head tail是gt实体的index
            for head, tail in zip(re_labels[b]["head"], re_labels[b]["tail"]):
                rel = {}
                # 判断gt_relations中的实体是否被识别出来
                if len(entities[b]["start"]) <= head or len(entities[b]["end"]) <= tail:
                    # rel_sent.append(rel)
                    continue

                rel["head_id"] = head
                rel["head"] = (entities[b]["start"][head], entities[b]["end"][head])
                rel["head_type"] = entities[b]["label"][head]

                rel["tail_id"] = tail
                rel["tail"] = (entities[b]["start"][tail], entities[b]["end"][tail])
                rel["tail_type"] = entities[b]["label"][tail]
                rel["type"] = 1
